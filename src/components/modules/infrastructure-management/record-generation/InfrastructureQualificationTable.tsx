...
import { useAllInfrastructureQualifications } from "@/hooks/modules/infrastructure-management/useInfrastructureQualification";
...
const InfrastructureQualificationTable: React.FC<{ title: string, pathName: string }> = ({ title, pathName }) => {
  const { data: infrastructureQualificationResponse, isLoading: isQualificationLoading } = useAllInfrastructureQualifications();
  ...
  // Get all infrastructure qualification IDs
  const qualificationIds = useMemo(() => {
    return infrastructureQualificationResponse?.data
      ?.map((item: any) => item[TABLE_CONSTANTS.INFRASTRUCTURE_QUALIFICATION.ID_FIELD])
      .filter((id: any) => id != null) ?? [];
  }, [infrastructureQualificationResponse]);

  // Fetch records for all infrastructure qualifications
  useEffect(() => {
    const fetchAllRecords = async () => {
      if (qualificationIds.length === NUMBERMAP.ZERO || !menuId) {
        setAllRecords([]);
        return;
      }
      ...
      try {
        const recordPromises = qualificationIds.map((id: number) =>
          getOrganizationRecords(menuId, RECORD_GENERATION_CONTEXT_TYPES.INFRASTRUCTURE_QUALIFICATION, id)
        );
  ...
  const isLoading = isQualificationLoading ?? isLoadingRecords;
...
export default InfrastructureQualificationTable;
